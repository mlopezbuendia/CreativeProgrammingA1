...
void setup() {
  ...
  //Load images
  imageMode(CENTER);
  marbles = new PImage[3];
  marbles[0] = loadImage("marble.png");
  marbles[1] = loadImage("marble1.png");
  marbles[2] = loadImage("marble2.png");
  userBall = loadImage("user.png");
  scoreBoard = loadImage("board.png");
...
void mouseDragged() {
  ...
  if (settingsComplete) {
    ...
    //Draw user ball
    image(userBall, mouseX, mouseY, userDiameter, userDiameter);
...
void mouseReleased() {
  ...
  if (settingsComplete) {
    ...
    for (int i=0; i<numEllipse; i++) {
      image(marbles[i % 3], ellipseX[i], ellipseY[i], badDiameter, badDiameter);
    }
    fill(normal);
...
void launchEllipse() {
  ...
  for (int i=0; i<numEllipse;i++) {
    ...
    for (int j=0; j<10; j++) {
      ...
      ellipseX[i] = constrain(ellipseX[i], badDiameter/2, width - badDiameter/2);
      ellipseY[i] = constrain(ellipseY[i], boxHeight, height - badDiameter/2);
...
void initEllipsePosition() {
  ...
  for (int i=0; i<numEllipse; i++) {
    ellipseX[i] = int(random(badDiameter, width-badDiameter));
    ellipseY[i] = int(random(badDiameter + boxHeight, height-badDiameter));
  }
